...
TOOLSDIR="$SRCDIR/Content/viewer/util/buildscripts"

# Destination directory for built code
DISTDIR="$BASEDIR/dist"

# Module ID of the main application package loader configuration
LOADERMID="Content/viewer/app/run"
...
if which node >/dev/null; then
  ulimit -n 1024
  node ../../dojo/dojo.js load=build --require "$LOADERCONF" --profile "$PROFILE" --releaseDir "$DISTDIR/Content/viewer" $@
...
cd "$DISTDIR/Content/viewer"
# rm -rf "dojo" # so that templateTests.html will work
rm -rf "dojox"
rm -rf "dijit"
